...
# (Optional) copy env defaults; sensible local-dev defaults are baked in
cp .env.example .env
...
The API listens on port `3690` and the Vite dev server proxies `/api/*` to it.
...
(see `.env.example`). Notable variables: `DATABASE_URL`, `PORT`,
