...
- The puzzle pages (`/games/:slug`) each get their own title server-side from the `puzzlePageTitles` table in `pkg/server/static.go`, mirroring the public `PUZZLES_BY_SLUG` registry (`app/components/library/crossword/puzzles.ts`) and the `usePageTitle(puzzle?.title)` call (`app/components/pages/Crossword.tsx`). Known public puzzles are indexable. Unknown and retired slugs are still served the React shell for the friendly puzzle-not-found page, but receive `noindex`. Add a row when you add a public puzzle, remove it when retiring one, and keep the titles in sync.
- Some routes are served `noindex` but still get a title so a shared link previews sensibly: the per-guest token/UUID links (`/i/:token`, `/u/:guestId`, matched by prefix since their tail is opaque) and the RSVP flow steps (`/rsvp/form`, `/rsvp/confirmation`, matched exactly). Those labels live in `noindexTitlePrefixes` / `noindexTitleExact` in `pkg/server/static.go`, mirroring the pages' `usePageTitle(...)` labels (keep them in sync). Like the public and puzzle pages they also get a self-referential `og:url`, derived from the path so a shared link's preview card points back to the page (nothing to keep in sync there), but no description.
- The info-collection link (`/i/:token`) is the one exception to the otherwise guest-data-free previews: its title is the primary guest's first name (`"<first name>'s Info"`), so the couple can tell which guest's link they're sharing apart in its unfurled card. The server resolves the full name from the token (`info.Service.PrimaryGuestName`) and takes the first name in the `/i/` branch of `injectMeta`; `"Your Details"` in `noindexTitlePrefixes` is only the fallback for when the name can't be resolved (unknown token, no named primary, no DB, or a lookup error). The client mirror is the `infoPageTitle(data)` helper in `InfoCollection.tsx` (a derived title, not a static `usePageTitle` label); keep the two first-name-possessive rules in sync. This stays safe because the opaque token in the shared URL already gates the whole party's data and the page stays `noindex`, so the name never enters a search index.
- The login-gated admin routes need no Go metadata-table entry: they are served `noindex` with the default title, since they are never shared and `useAdminPageTitle` handles the tab title client-side. They still need a `frontendRoutes` entry and route-inventory test case so direct navigation receives the shell.
