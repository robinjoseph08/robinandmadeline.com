...
// the service generates one (5 uppercase letters from an unambiguous alphabet),
// when set it must be unique. Address fields are optional at creation; whether
// they are required is a status concern, not a creation precondition, so an
// under-filled party can be created and reads incomplete.
//
...
// present-but-blank value is a 422, so a code arrives either non-blank or as
// JSON null, which asks the service to generate one. Code-less (SQL NULL)
// parties arise only from the PATCH clear gesture, which the partial unique
// index permits in any number. circle defaults to [] so a nil slice stores
// '{}', not NULL.
