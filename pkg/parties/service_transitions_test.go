...
func completePhysicalParty(t *testing.T, svc *parties.Service) *parties.Party {
	t.Helper()
...
func TestMarkComplete_Rejected422WhenRequiredFieldsMissing(t *testing.T) {
	...
	assert.ErrorIs(t, err, parties.ErrRequiredFields)
}
...
func TestMarkComplete_RejectedWhenPrimaryEmailMissing(t *testing.T) {
	...
	addGuestT(t, svc, p.ID, parties.CreateGuestInput{FullName: "No Email", IsPrimary: true})
...
func TestMarkComplete_SetsRequestedAndConfirmed(t *testing.T) {
	...
	assert.Equal(t, parties.StatusComplete, parties.StatusOf(marked))
}
...
func TestRequestInfo_SetsRequestedAndResetsConfirmed(t *testing.T) {
	...
	assert.Equal(t, parties.StatusIncomplete, parties.StatusOf(reopened),
...
func TestStatus_DerivedBeforeRequested(t *testing.T) {
	...
	p := createPartyT(t, svc, digitalPartyInput())
	addGuestT(t, svc, p.ID, parties.CreateGuestInput{FullName: "Has Email", Email: ptr("has@example.com"), IsPrimary: true})

	reloaded, err := svc.GetParty(ctx(), p.ID)
	require.NoError(t, err)
	assert.False(t, reloaded.InfoCollectionRequested)
	assert.Equal(t, parties.StatusComplete, parties.StatusOf(reloaded))
}
...
func TestSubmitInfoForm_GatedAndConfirms(t *testing.T) {
	...
	require.ErrorIs(t, err, parties.ErrRequiredFields)
	...
	assert.Equal(t, parties.StatusComplete, parties.StatusOf(submitted))
}
