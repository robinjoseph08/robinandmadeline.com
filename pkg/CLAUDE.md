...
- Struct tags are the spec. Validate with `validate` (`required`, `omitempty`, `oneof=...`, `min`/`max`, `email`, `dive`, the custom `date`/`time`/`datetimeblank`/`url`/`httpurl`/`emailblank`/`phone`, where `httpurl` is the http(s)-only sibling of `url` for values rendered as an anchor href); normalize with `mod` (`trim`, and `dive` to reach slice elements); fill with `default` (e.g. `default:"[]"` to make a nil slice store `'{}'`). Query filters add a `query:"..."` alias (gorilla/schema) and stay pointers so "absent" is distinguishable.
