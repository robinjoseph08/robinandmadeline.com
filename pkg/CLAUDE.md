...
- Build optional-field and test-fixture pointers with `golib/pointerutil` (`String`/`Int`/`Bool`/`Float64`/`Time`, plus `Equal`/`EqualSlices`). Always prefer a `pointerutil` constructor over writing a new pointer helper or using inline `&v`; if you need one it does not have, add it to `golib` rather than rolling a local helper. Note `pointerutil.EmptyString` maps `""` to nil, so it is not used for fields like `rsvp_code`/address that rely on explicit JSON null plus validation.
