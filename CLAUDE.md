...
- **SPA route allowlist**: production serves the React shell only for known frontend route shapes and returns a real server-side 404 for everything else (ADR 0010). Whenever a route is added, removed, or reshaped in `app/router.tsx`, update the server allowlist and its tests in the same change. Keep this as an allowlist; never accumulate denylisted scanner paths such as `*.php`.
