...
Whether a party's info collection is `complete` or `incomplete`; a party can be `complete` only when all required fields are present.
_Avoid_: "complete" as a synonym for "has all required fields": the fields are necessary but, once collection is requested, not sufficient.
