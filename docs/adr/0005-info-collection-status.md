...
A party's **info-collection status** (`complete`/`incomplete`) is derived from whether its required fields are present, until the couple delegates collection by sending the info link, after which it stays `incomplete` until the guest submits the info form or the couple manually marks it complete. A party can be `complete` only if all required fields are present (the primary guest's email for every party, plus a full mailing address for physical parties), so an under-filled party can never be marked complete. Sending the link always resets status to `incomplete`, and editing fields by hand never changes status once collection has been requested.
...
- The "required fields" set is invitation-type-specific and is the single gate on completion, so the info-collection form must enforce exactly those fields (primary email everywhere; full mailing address for physical parties).
