# Database-backed email queue with exactly-once delivery

Bulk emails (~150 recipients) are sent through a database-backed queue rather than synchronously in the request handler or via an external queue service. Each `email_recipients` row moves `queued → sending → sent`, and on startup any stale `sending` rows are checked against the Mailgun API before retrying.
