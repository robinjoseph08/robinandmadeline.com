...
- Subscription is enforced twice: unsubscribed guests are excluded at enqueue (and shown in the compose preview's "unsubscribed" bucket), and the worker re-checks the guest before each send. Because a large send can span two days at the 100/day send cap, a mid-send unsubscribe is possible, so a worker-skipped row is recorded with a dedicated `unsubscribed` recipient status to keep it distinct from a delivery `failed`.
